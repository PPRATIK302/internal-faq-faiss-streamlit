...
def list_sources(docs: List[Document]) -> str:
    seen = set()
    lines = []
    for d in docs:
        meta = d.metadata or {}
...
def main():
    st.set_page_config(page_title="Internal FAQ RAG – PlaceMakers", layout="wide")
    st.title("🔍 Internal FAQ Assistant – PlaceMakers LEARN")
    ...
    st.sidebar.header("1️⃣ Upload & Index Corpus")
    uploaded_file = st.sidebar.file_uploader(
        "Upload placemakers_learn_corpus.txt",
        type=["txt"],
        help="Use the TXT generated by your scraper.",
    )

    if st.sidebar.button("Upload & Index to Vector DB"):
        if uploaded_file is None:
            st.sidebar.warning("Please upload a .txt file first.")
        else:
            file_bytes = uploaded_file.getvalue()
            with st.spinner("Building/loading FAISS index..."):
                vectordb, info = load_or_build_vectordb(file_bytes)

            st.session_state["vectordb"] = vectordb
            st.session_state["index_info"] = info

            mode = info.get("mode", "ready")
            articles = info.get("articles_indexed", "N/A")
            sha16 = info.get("sha16", "N/A")
            st.sidebar.success(f"FAISS {mode}. Articles: {articles} | Hash: {sha16}")

    st.sidebar.markdown("---")
    if st.session_state.get("index_info"):
        st.sidebar.caption(f"Index dir: {st.session_state['index_info'].get('index_dir','')}")
    st.sidebar.caption("Index must be ready before asking questions.")
    ...
    if st.button("Get Answer"):
        ...
        if vectordb is None:
            st.error("No vector DB loaded. Please upload & index the corpus first.")
            return
        ...
        st.markdown("### 🧠 Retrieval Decisions")
        st.write(
            {
                "intent": intent,
                "category": category,
        ...
        st.markdown("### 📚 Context Used (Retrieved Chunks)")
        st.caption("Below are the exact chunks used from your vector DB:")

        for i, d in enumerate(docs, start=1):
            meta = d.metadata or {}
            title = meta.get("title", "Untitled")
            url = meta.get("url", "no-url")
            category_ = meta.get("category", "unknown")

            with st.expander(f"Chunk {i}: {title}"):
                st.markdown(f"**Category:** {category_}")
                st.markdown(f"**URL:** {url}")
                st.markdown("---")
                st.write(d.page_content)

        st.markdown("### 🔗 Source Articles")
        st.text(list_sources(docs))
